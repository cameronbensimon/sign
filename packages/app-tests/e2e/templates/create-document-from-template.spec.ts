...
test('[TEMPLATE]: should create a document from a template', async ({ page }) => {
  ...
  await page.getByLabel('Redirect URL').fill('https://tiquo.co');
  await page.getByRole('button', { name: 'Continue' }).click();

  await expect(page.getByRole('heading', { name: 'Add Placeholder' })).toBeVisible();

  // Add 2 signers.
  await page.getByPlaceholder('Email').fill('recipient1@tiquo.co');
  await page.getByPlaceholder('Name').fill('Recipient 1');
  await page.getByRole('button', { name: 'Add Placeholder Recipient' }).click();
  await page.getByPlaceholder('Email').nth(1).fill('recipient2@tiquo.co');
  ...
  expect(document.documentMeta?.redirectUrl).toEqual('https://tiquo.co');
...
test('[TEMPLATE]: should create a document from a template with custom document', async ({
  ...
  // Add a signer
  await page.getByPlaceholder('Email').fill('recipient@tiquo.co');
